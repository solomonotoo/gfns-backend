...
//		IncidentAdd incident = mapper.map(dto, IncidentAdd.class);
//		
//		if (dto.getRegion() != null && dto.getRegion().getId() != null) {
//			Region region = regionRepo .findById(dto.getRegion().getId())
//				    .orElseThrow(() -> new RuntimeException("Invalid region "));
//			incident.setRegion(region);
//        } else {
//        	incident.setRegion(null);
//        }
//		
//		if (dto.getIncidentCategory() != null && dto.getIncidentCategory().getId() != null) {
//			IncidentCategory category = categoryRepo .findById(dto.getIncidentCategory().getId())
//				    .orElseThrow(() -> new RuntimeException("Invalid Incident Category "));
...
//		
//
//		if (dto.getIncidentType() != null && dto.getIncidentType().getId() != null) {
//			IncidentType incidentType = typeRepo.findById(dto.getIncidentType().getId())
//				    .orElseThrow(() -> new RuntimeException("Invalid incident type "));
...
public class IncidentAddServiceImpl implements IncidentAddService {
    ...
    @Override
    public IncidentAddResponseDTO updateIncidentAdd(Long id, IncidentAddRequestDTO dto) {
        IncidentAdd existingIncidentAdd = getIncidentAddById(id);
        
        IncidentAdd incidentToUpdate = convertFromDTOToEntity(dto);
        incidentToUpdate.setId(id);

        // Map scalar fields only (ModelMapper is configured to skip relations)
        mapper.map(incidentToUpdate, existingIncidentAdd);
        IncidentAdd updated = incidentAddRepo.save(existingIncidentAdd);
        return convertFromEntityToDTO(updated);
    }
    ...
    private IncidentAdd convertFromDTOToEntity(IncidentAddRequestDTO dto) {
        ...
        // Relations are skipped by ModelMapper, so handle here if needed
        if (dto.getRegion() != null && dto.getRegion().getId() != null) {
            Region region = regionRepo.findById(dto.getRegion().getId())
                    .orElseThrow(() -> new RuntimeException("Invalid region"));
            incident.setRegion(region);
